...
def main():
    ...
    # import images.csv
    image_full = pd.read_csv(pathlib.Path("../../data_dump/images_cleaned.csv"))
    image_full["image_path"] = image_full["image_name"].apply(lambda x: \
        str(pathlib.Path("../../raw_data/suumo_images") / str(x).split("_")[0] / x))
    images = list(image_full["image_path"])[100:102]
    ...
    return print(labels, images, cluster_names)
